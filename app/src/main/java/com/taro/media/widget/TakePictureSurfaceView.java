...
import com.taro.media.camera.CameraController;
import com.taro.media.camera.CameraHelper;
import com.taro.media.camera.CameraRecordRenderer;
import com.taro.media.camera.CameraSurfaceRenderer;
import com.taro.media.camera.CommonHandlerListener;
import com.taro.media.camera.TakePictureSurfaceRenderer;
import com.taro.media.filter.FilterManager;

@TargetApi(11)
public class TakePictureSurfaceView extends AutoFitGLSurfaceView implements CommonHandlerListener, SurfaceTexture.OnFrameAvailableListener {

    private CameraHandler mBackgroundHandler;
    private HandlerThread mHandlerThread;
    private TakePictureSurfaceRenderer mCameraRenderer;
    ...
    private void init(Context context) {
        ...
        mCameraRenderer = new TakePictureSurfaceRenderer(context.getApplicationContext(), mBackgroundHandler);

        setRenderer(mCameraRenderer);
        setRenderMode(RENDERMODE_WHEN_DIRTY);
    }

    public TakePictureSurfaceRenderer getRenderer() {
                ...
                mBackgroundHandler.post(new Runnable() {
                    @Override public void run() {
                        ...
                        mBackgroundHandler.sendMessage(mBackgroundHandler.obtainMessage(
                                TakePictureSurfaceView.CameraHandler.CONFIGURE_CAMERA, width, height));
                    }
                });
            }
            break;
